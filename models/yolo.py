...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, TSCODE_Detect):
            ...
            # print('Strides: %s' % m.stride.tolist())
        if isinstance(m, (IDetect, IDetect_AFPN_P345, IDetect_AFPN_P2345, IDetect_Decoupled_DyHead, IDetect_Decoupled)):
            s = 640  # 2x min stride
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(m, (IDetect, IDetect_AFPN_P345, IDetect_AFPN_P2345, IDetect_Decoupled_DyHead, IDetect_Decoupled)) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint) or isinstance(m, TSCODE_Detect):
                    break

            if profile:
                c = isinstance(m, (Detect, IDetect, IDetect_AFPN_P345, IDetect_AFPN_P2345, IDetect_Decoupled_DyHead, IDetect_Decoupled, IAuxDetect, IBin, TSCODE_Detect))
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            ...
            elif isinstance(m, (IDetect, IDetect_AFPN_P345, IDetect_AFPN_P2345, IAuxDetect, IDetect_Decoupled_DyHead, IDetect_Decoupled)):
                m.fuse()
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
                 ...
                 Yolov7x_E_ELAN, Yolov7_Tiny_E_ELAN_Attention, Yolov7_Tiny_E_ELAN_DCN,
                 Yolov7_Tiny_E_ELAN_DBB, DiverseBranchBlock, VoVGSCSP, GSConvns]:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='cfg/training/yolov7-tiny-rexnet.yaml', help='model.yaml')
